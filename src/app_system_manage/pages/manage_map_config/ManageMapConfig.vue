...
import OffcanvasMapConfig from "./OffcanvasMapConfig.vue";
import { useMapConfigStore } from "./mapConfigStore.js"

const address = {
  title: props.nameKCN,
  details: [{ name: props.nameKCN, to: "#" }],
};
...
const currentMapConfig = mapConfigStore.currentMapConfig
...
const handleClickDetail = ($event) => {
  const item = getValueItem($event)
  mapConfigStore.actionType = "detail"
  currentMapConfig.value = item
}
...
<template>
  <Layout :address="address" :items="address.details" chooseMenu="ban-do" :slug="props.slug">
    ...
    <OffcanvasMapConfig></OffcanvasMapConfig>
  </Layout>
</template>
