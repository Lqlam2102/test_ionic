...
const addNewSuggest = () => {
    let isContain = false
    mapConfigToEdit.config.suggestion.forEach(suggest => {
        if (suggest.type === newSuggest.value.type && suggest.icon === newSuggest.value.icon) {
            isContain = true
        }
    })
    if (!isContain) {
        mapConfigToEdit.config.suggestion.push(newSuggest.value)
        resetNewSuggest()
    }
    else {
...
watch(mapConfigToEdit, () => {
    mapConfigStore.mapConfigToEdit.value = mapConfigToEdit
})
                ...
                <b-row>
                    ...
                    <b-col lg="6" class="mb-2">
                        ...
                        <input type="text" class="form-control" id="nameMapConfigInput" placeholder="Nhập tên cấu hình"
                            v-model="mapConfigToEdit.name" autocomplete="new-address" :class="{ 'is-invalid': !mapConfigToEdit.name }" />
                        <div class="invalid-feedback d-block" v-if="!mapConfigToEdit.name">
                    ...
                    <b-col lg="6" class="mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input fs-15" type="checkbox" role="switch" id="isDefaultMapInput" v-model="mapConfigToEdit.is_main">
                    ...
                    <b-col lg="6" class="mb-2">
                        ...
                        <input type="text" class="form-control" id="centerMapInput" placeholder="10.336505,106.368963"
                            name="centerMap" v-model="mapConfigToEdit.config.center" autocomplete="new-centerMap" />
                    </b-col>
                    <b-col lg="6" class="mb-2">
                        <label for="typeMapInput" class="form-label mb-1">Loại bản đồ</label>
                        <select id="typeMapInput" class="form-control" v-model="mapConfigToEdit.config.type">
