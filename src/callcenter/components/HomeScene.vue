<template>
    <!-- PAGE DIAPAD -->
    <section id="page-diapad" class="w-100 margin-top-40">
        <!-- Typing Number Section -->
        <div class="wrap-typing-number pl-15 pr-15 mt-10 mb-30 d-flex justify-content-between">
            <input type="text" v-model="typedNumber" placeholder="+840000000000"
        ...
        <div class="wrap-diapad mb-10">
            <div class="diapad-row" v-for="(row, rowIndex) in dialpadKeys" :key="rowIndex">
                <button class="diapad-key" v-for="(key, keyIndex) in row" :key="keyIndex"
        ...
        <div class="wrap-call-using-dropdown position-relative" v-if="showDropdown">
            <div id="list-from-numbers" class="call-using-dropdown box-shadow3 border-radius-8 bg-white">
                <div class="call-using-dropdown-item cursor-pointer p-15 pt-10 pb-10"
                    v-for="(number, index) in fromNumbers" :key="index" @click="selectFromNumber(number)">
                    <div>
...
export default {
    ...
    computed: {
        selectedFromNumber() {
            return {
                name: "Number 1",
                phone: this.callCenter.fromNumber,
            }
        },
        fromNumbers() {
            return [{ name: "Number 1", phone: this.callCenter.fromNumber }]
        }
    },
    methods: {
...
<style scoped src="@/callcenter/assets/stringee-widget/css/all-style-1.0.0.css"></style>
<style scoped>
.diapad-key-number {
    height: 37px;
}
