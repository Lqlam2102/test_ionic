<template>
  <section id="page-calling" class="w-100 margin-top-40">
      <!-- Status Section -->
      <section class="wrap-status p-l-r-20 top-0 position-relative d-flex">
          ...
          <div class="wrap-status-time line-height-30 position-absolute right-0">
              <span class="status-time">{{ callCenter.getFormattedCallDuration() }}</span>
          </div>
      </section>

      <!-- Info Section -->
      <section class="wrap-info p-20">
          <div class="info-name pb-10" v-if="callCenter.incomingCallVisible || callCenter.answered">
              {{ callCenter.fromNumber ?? callCenter.baseNumber }}
          </div>
          <div class="info-name pb-10" v-else>
                {{ callCenter.toNumber ?? callCenter.baseNumber }}
          </div>
      ...
      <section class="wrap-action d-flex justify-content-between">
          <button id="btnMic" class="action-call text-center flex-basis p-10 bg-white border-0">
              <img class="icon" :src="micIcon" />
              <img class="icon-on d-none" :src="micOffIcon" />
          </button>
          <button id="btnHold" class="action-call text-center flex-basis p-10 bg-white border-0">
