...
function formatPercent(p) {
  ...
  return `${p.toFixed(2)}%`; // Giữ lại 2 chữ số thập phân
}
...
function getWidth(campaign) {
  ...
  const ratio = Math.min(campaign.worker_count / campaign.total_kpi, 1) * 100; // Nhân với 100 để chuyển thành phần trăm
...
const getDeltaPercent = (campaign) => {
  ...
  // Tính phần trăm và giữ lại 2 chữ số thập phân
  return ((worker_count / total_kpi) * 100).toFixed(2);
};
</script>
