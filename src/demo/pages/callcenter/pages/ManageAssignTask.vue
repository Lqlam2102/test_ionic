...
export default {
    ...
    data() {
        return {
            ...
            departmentMembersAPI: API_SERVER_URL + '/user/get-list-users-department?all',
            candidateData: [],
            candidatePreload: [],
    ...
    computed: {
        isLoading() {
            return this.isFetchingCandidateData || this.isFetchingDepartmentMembers || this.isFetchingPreloadData
        },
        data() {
    ...
    methods: {
        async fetchCandidateData() {
            ...
            await useFetch(this.candidateDataAPI)
                .then((data) => { this.candidateData = data; })
        ...
        async fetchDepartmentMembers() {
            ...
            await useFetch(this.departmentMembersAPI)
                .then((data) => { this.departmentMembersObject = data; })
        ...
        async fetchPreloadData() {
            ...
            await useFetch(this.candidatePreloadAPI)
                .then((data) => { this.candidatePreload = data; })
                .catch((error) => { console.error("Error fetching candidate preload data:", error); });
        ...
        async handleSubmitAssignTask() {
            let selectedCandidates = this.data.filter(item => item.select).map(item => item.id)
...
<template>
    <Layout :address="{ title: 'QUẢN LÝ ỨNG VIÊN / CHIA SỐ' }" chooseMenu="ban-do">
        <div class="chat-wrapper gap-xl-1 mx-n4 p-1 h-100 d-flex flex-column">
            <div class="card mb-0 flex-grow-1 position-relative p-3" style="overflow: hidden auto">
                ...
                <template v-else>
                    <div class="w-100 d-flex justify-content-start mb-3">
                        <Select defaultValueLabel="Người chăm sóc" isCloseOnSelect="true" :localData="departmentMembers" searchField="str" labelField="str" style="min-width: 300px;"
                            @change-data="(data) => selectedUser = data"></Select>
                        <button class="btn btn-primary ms-2" type="button" @click="handleSubmitAssignTask">
                    ...
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center" v-for="(attr, index) in listAttributes" :key="index">{{
                                    attr.name }}
                                </th>
                            </tr>
                        </thead>
                        <tbody class="position-relative">
                            <tr v-for="(record, index) in data" :key="index" class="form__user-data">
                                <td class="text-center">
                                    <input type="checkbox" class="cursor-pointer" v-model="record.select"
                                        :id="record.id" />
                                </td>
                                <td v-for="(attr, index) in listAttributes" :key="index" class="text-center">
