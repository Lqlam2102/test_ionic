  ...
  <div class="table-responsive mt-1 pb-4 table-card">
    ...
    >
      <thead class="text-muted table-light">
        <tr>
          <th v-for="col in columns" :key="col.label" class="text-center">
            {{ col.label }}
          </th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(user, idx) in records" :key="user.user_id || idx">
          <td v-for="col in columns" :key="col.label" class="text-center">
...
const records = computed(() => {
  return [...props.data]
    .sort((a, b) => {
      const aCandidates = Number(a.total_candidates?.current) || 0;
      const bCandidates = Number(b.total_candidates?.current) || 0;
...
const compareRecords = computed(() => {
  return [...props.data]
    .sort((a, b) => {
      const aCandidates = Number(a.total_candidates?.compare) || 0;
      const bCandidates = Number(b.total_candidates?.compare) || 0;
      return bCandidates - aCandidates;
    })
    .sort((a, b) => {
      const aWorkers = Number(a.total_workers?.compare) || 0;
      const bWorkers = Number(b.total_workers?.compare) || 0;
      return bWorkers - aWorkers;
    });
});
...
const rankingMap = computed(() => {
  return records.value
    ...
    .sort(
      (a, b) =>
        (b.total_workers?.current || 0) - (a.total_workers?.current || 0)
    )
    .reduce((map, u, idx) => {
      map[u.user_id] = idx + 1;
      return map;
...
const columns = [
  ...
  },
  {
    ...
    formatter: (u) => u.total_interview_passed?.current || 0,
  },
  {
    label: "Tổng phút gọi",
    formatter: (u) =>
  ...
  },
  {
    ...
    formatter: (u) =>
      u.total_kpi?.percent_change != null ? u.total_kpi.percent_change : 0,
  },
  {
    label: "Xếp hạng",
